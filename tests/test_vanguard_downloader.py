...
def test_parse_price():
    ...
    assert price_obj1.date == "2023-10-26"
    assert price_obj1.value == 12345
    assert price_obj1.denom == 10000
    assert price_obj1.currency == "AUD"

    # Test case 2: Price with two decimal places
    price_obj2 = dl._parse_price("2023-10-27", "56.78", "USD")
    assert price_obj2.date == "2023-10-27"
    assert price_obj2.value == 5678
    assert price_obj2.denom == 100
    assert price_obj2.currency == "USD"

    # Test case 3: Integer price
    price_obj3 = dl._parse_price("2023-10-28", "123", "EUR")
    assert price_obj3.date == "2023-10-28"
    assert price_obj3.value == 123
    assert price_obj3.denom == 1 
    assert price_obj3.currency == "EUR"
    
    # Test case 4: Price like "0.987"
    price_obj4 = dl._parse_price("2023-10-29", "0.987", "AUD")
    assert price_obj4.date == "2023-10-29"
    assert price_obj4.value == 987
    assert price_obj4.denom == 1000
