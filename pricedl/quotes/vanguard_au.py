...
class VanguardAuDownloader:
    ...
    async def _dl_fund_data(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
        """
        Fetches retail fund prices data from the Vanguard API.
        Returns the parsed 'fundData' dictionary.
        """
        logging.debug(f"Fetching fund data from {self._API_URL}")
        ...
        if content.startswith("callback(") and content.endswith(")"):
            logging.debug("Cleaning up JSONP callback wrapper.")
        ...
        if "fundData" not in parsed_json:
            logging.error("'fundData' not found in JSON response.")
    ...
    def _get_fund_info(self, fund_data: Dict[str, Any], fund_id: str) -> FundInfo:
        ...
        try:
            ...
            logging.error(f"Missing expected key in fund info for {fund_id}: {e}")
            raise ValueError(f"Data for fund {fund_id} is incomplete: missing {e}") from e
    ...
    def _parse_price(self, fund_info: FundInfo) -> Price:
        ...
        try:
            ...
            logging.error(f"Failed to parse date string '{fund_info.date}': {e}")
        ...
        try:
            ...
            logging.error(f"Failed to parse decimal value '{value_str}': {e}")
            raise ValueError(f"Invalid price value format '{value_str}'") from e

        sign, digits_tuple, exponent = decimal_val.as_tuple()

        if not digits_tuple and exponent == 0 and decimal_val.is_zero(): # Handles Decimal('0')
             mantissa_int = 0
        else:
...
async def main_test():
    ...
    try:
        ...
        assert price.value != 0 or price.denom != 0, "Price value or denom should not be zero (unless actual price is 0)"
        assert price.symbol == symbol
        print("\nTest assertions passed for VANGUARD:HY.")

    except ValueError as ve:
        print(f"\nConfiguration or data error for {test_symbol_str}: {ve}")
        logging.error(f"ValueError for {test_symbol_str}: {ve}", exc_info=True)
    except aiohttp.ClientError as ce:
        print(f"\nNetwork or API error for {test_symbol_str}: {ce}")
        logging.error(f"ClientError for {test_symbol_str}: {ce}", exc_info=True)
    except Exception as e:
        print(f"\nAn unexpected error occurred for {test_symbol_str}: {e}")
        logging.error(f"Unexpected error for {test_symbol_str}: {e}", exc_info=True)
    ...
    try:
        ...
        await downloader.download(symbol_unknown, "AUD")
    except ValueError as e:
    ...
    try:
        ...
        await downloader.download(symbol_invalid_ns, "AUD")
    except ValueError as e:
...
if __name__ == "__main__":
    # To run this example, you'll need to install aiohttp:
    # pip install aiohttp
